...
// declare head of linked list for Phase 3
...
void* pong_thread(void* threadarg) {
	...
	// Below is the code we replaced with our code above.
    /*
	...
	*/
	...
	// Phase 4:
    // Handle congestion by waiting for the time indicated by the request
    if (result > 0) {
		pthread_mutex_lock(&mutex);
		usleep(result*1000);
		pthread_mutex_unlock(&mutex);
	...
	// Phase 3:
	// Add connections to our linked list if their state is HTTP_DONE
	// Otherwise close it
